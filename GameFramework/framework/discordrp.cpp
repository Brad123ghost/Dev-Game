...
int DiscordRP::Initialize()
{
    ...
    m_state.core->SetLogHook(
        discord::LogLevel::Debug, [](discord::LogLevel level, const char* message) {
            std::cerr << "Log(" << static_cast<uint32_t>(level) << "): " << message << "\n";
        });
    m_pcore->UserManager().GetCurrentUser(&m_state.currentUser);
    m_pcore->UserManager().OnCurrentUserUpdate.Connect([this]() {
        m_state.core->UserManager().GetCurrentUser(&m_state.currentUser);

        std::cout << "Current user updated: " << m_state.currentUser.GetUsername() << "\n";
        ...
        handle.SetId(m_state.currentUser.GetId());
        handle.SetType(discord::ImageType::User);
        handle.SetSize(256);

        m_state.core->ImageManager().Fetch(
            handle, true, [this](discord::Result res, discord::ImageHandle handle) {
                if (res == discord::Result::Ok) {
                    discord::ImageDimensions dims{};
                    m_state.core->ImageManager().GetDimensions(handle, &dims);
                    ...
                    uint8_t* d = data.data();
                    m_state.core->ImageManager().GetData(handle, d, static_cast<uint32_t>(data.size()));
    ...
    /*   state.core->ActivityManager().OnActivityJoin.Connect(
       ...
       state.core->LobbyManager().OnSpeaking.Connect(
        ...
        m_sdetails = "Developing";
        m_sstate = "Solo King!";

    discord::Activity activity{};
    activity.SetDetails(m_sdetails.c_str());
    activity.SetState(m_sstate.c_str());
    //activity.GetAssets().SetSmallImage("yepper");
    //activity.GetAssets().SetSmallText("i mage");
    activity.GetAssets().SetLargeImage("yepper");
    ...
    m_state.core->ActivityManager().UpdateActivity(activity, [](discord::Result result) {
...
void DiscordRP::UpdateActivity(char* details, char* state)
{
    m_sdetails = details;
    m_sstate = state;
